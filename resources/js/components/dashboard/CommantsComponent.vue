<template>
  <div>
    <header class="bg-white space-y-4 p-4 sm:px-8 sm:py-6 lg:p-4 xl:px-8 xl:py-6">
      <div class="flex items-center justify-between">
        ...
        <div class="group flex items-center">
          <button class="mx-2 rounded-md text-red-500 border border-red-500 hover:border-red-400 hover:bg-red-50 text-sm font-medium px-3 py-2 shadow-sm" @click="bulkDelete">Bulk Delete</button>
          <router-link class="rounded-md text-blue-500 border border-blue-500 hover:border-blue-400 hover:bg-blue-50 text-sm font-medium px-3 py-2 shadow-sm" :to="{ name: 'user.post.index' }">My Posts</router-link>
        </div>
      </div>
        ...
        <thead>
          <tr>
            <th class="px-6 py-3 bg-gray-50">
              <span class="text-xs font-medium tracking-wider leading-4 text-left text-gray-500">Select Ids</span>
            </th>
            <th class="px-6 py-3 bg-gray-50">
              <span class="text-xs font-medium tracking-wider leading-4 text-left text-gray-500">Comment</span>
            </th>
            <th class="px-6 py-3 bg-gray-50">
              <span class="text-xs font-medium tracking-wider leading-4 text-left text-gray-500">User</span>
            </th>
            <th class="px-6 py-3 bg-gray-50">
              <span class="text-xs font-medium tracking-wider leading-4 text-left text-gray-500">Creation Date</span>
            </th>
          </tr>
        </thead>
    ...
    <div v-else>
      <h2 class="text-center py-12 font-bold">No comments found.</h2>
    </div>
  </div>
</template>
...
const bulkDelete = async () => {
  const confirmed = await Swal.fire({
    title: "Are you sure?",
    text: "You won't be able to revert this comments!",
    icon: "warning",
